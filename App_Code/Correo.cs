...
public class Correo
{
    ...
    public int enviarEventoAdministrador(string email, string nombre, string nombreUsuario, string apellidoUsuario, string nombreEvento, string fechaEvento, string horaEvento, string claveUsuario)
    {
        MailAddress correo = new MailAddress(email, nombre); //correo del cliente
        MailAddress correoServidor = new MailAddress("dsiv.uaq@hotmail.com", "Universidad Autónoma de Querétaro"); // correo del servidor
        ...
        MM.Body = "El siguiente usuario ha solicitado la reservación de una sala:<br />"
            ...
            + "Fecha: " + fechaEvento + "<br />"
            + "Hora: " + horaEvento + "<br />"
            + "<a href=\"" + directorioServidor + "Login/Verificacion.aspx?ValidarEvento=" + claveUsuario + "\" >Click aquí para habilitar la cuenta del usuario.</a>";
        MM.IsBodyHtml = true;
        MM.Priority = MailPriority.High; //prioridad del correo
